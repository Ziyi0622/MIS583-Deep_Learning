...
def count_tensor_elements(x: Tensor) -> int:
    ...
    if shape:           
        num_elements = 1  # If shape exists, start by assuming at least 1 element
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    rounded_start = (start + 9) // 10 * 10
    
    if rounded_start >= stop:
        x = torch.tensor([], dtype=torch.float64)  
    
    else: 
        x = torch.arange(rounded_start, stop, 10, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    Returns:
        ...
        - first_two_rows_three_cols: Tensor of shape (2, 3) giving the data in
          the first two rows and first three columns of x. dk0Fl1P67
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
     # Calculate N (number of elements in the list) and C (max value in x + 1)
    N = len(x)
    C = max(x) + 1 
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    their instance method variants x.mean, x.std. dk0Fl1P67
    ...
    means = x.sum(dim=0) / x.size(0)
    var = ((x - means) ** 2).sum(dim=0) / x.size(0)
    std = var.sqrt()
